...
load(file = "Base_definitiva.RData")
```
...
df %>% group_by(MES,CLASE, PERIODO) %>% 
  summarise(total = n()) %>% 
  ggplot(aes(x = MES, y = total , color = CLASE)) +
  geom_line() +
  geom_point() +
  facet_wrap(~PERIODO) + 
...
```{r, include=F, eval = F}
mse_comuna_dia <- data.frame(Modelo_Mixto_Poisson = c("ACCIDENTALIDAD ~ 1 + (1 | COMUNA/CLASE)",
...
mse_comuna_dia <- mse_comuna_dia %>%
  mutate(Porcentaje_Variacion = round(100*abs(Train_MSE-Test_MSE)/(Train_MSE+Test_MSE), 2))
```

```{r, include = F}
load(file = "mse_comuna_dia.RData")
```


```{r, echo = FALSE}
knitr::kable(mse_comuna_dia)
```
...
```{r, include=F}
remove(mod_dia_comuna0, mod_dia_comuna1, mod_dia_comuna2, mod_dia_comuna3, mod_dia_comuna4)
```
...
```{r, include=F}
remove(mod_semana_comuna0, mod_semana_comuna1, mod_semana_comuna2, mod_semana_comuna3)
```
...
```{r, include=F}
remove(mod_mes_comuna0, mod_mes_comuna1, mod_mes_comuna2)
```
...
```{r, eval=F, include=F}
mse_barrio_dia <- data.frame(Modelo_Mixto_Poisson = c("ACCIDENTALIDAD ~ DIA_FESTIVO + (1 | BARRIO/CLASE)"),
                        Train_MSE = c(mse_train_dia_barrio),
                        Test_MSE = c(mse_test_dia_barrio))
mse_barrio_dia <- mse_barrio_dia %>%
  mutate(Porcentaje_Variacion = round(100*abs(Train_MSE-Test_MSE)/(Train_MSE+Test_MSE), 2))
```

```{r, include=F}
load(file = "mse_barrio_dia.RData")
```

```{r, echo = FALSE}
knitr::kable(mse_barrio_dia)
```
...
```{r, include=F, eval=F}
mse_barrio_semana <- data.frame(Modelo_Mixto_Poisson = c("ACCIDENTALIDAD ~ DIA_FESTIVO + (1 | BARRIO/CLASE)"),
                        Train_MSE = c(mse_train_semana_barrio),
                        Test_MSE = c(mse_test_semana_barrio))
mse_barrio_semana <- mse_barrio_semana %>%
  mutate(Porcentaje_Variacion = round(100*abs(Train_MSE-Test_MSE)/(Train_MSE+Test_MSE), 2))
```

```{r, include = F}
load(file = "mse_barrio_semana.RData")
```


```{r, echo = FALSE}
knitr::kable(mse_barrio_semana)
```
...
'''
Según el boletín 070 del concejo de Medellín, se dice que en el 2017 murieron 258 personas por incidentes de transito, por lo cual, se está convirtiendo en un problema de salud pública. La ciudad ocupa el primer lugar de accidentalidad en todo el territorio nacional, por dichos motivos, sugieren que que es muy importante trabajar en proyectos que se encarguen de velar por la seguridad de las personas en la vía, y, si es necesario, brindar sanciones ejemplares.
'''

Según los descrito en la introcucción, es necesario centrarse en los métodos preventivos más que en las sanciones, debido a que, si la población conoce sus cifras y logra entender qué es lo que esta pasando dentro de la ciudad, se logrará no sólo reducir los incidentes de tránsito, sino también crear conciencia en las generaciones futuras.
...
Considerando las caracteristicas de Medellín y del valle de Aburrá en general, pueden existir muchos factores que catalogan un barrio, en cuanto a seguridad vial, como peligroso o seguro. Día a día en la ciudad transitan todo tipo de vehículos, que dadas las condiciones de la zona, las calles angostas en algunos sectores, las vías en estado de deterioro y la imprudencia de la gente, hacen que sea muy complicado ir de un lugar a otro sin tener algún tipo de accidente de tránsito en algún momento de la vida de una persona. Por este motivo, el criterio escogido para clasificar un barrio en vista de se accidentalidad fue la gravedad del accidente. 

En primer lugar, la gravedad del accidente habla mucho de las condiciones en las que se dio el mismo, ya que muchas veces un mayor daño es causado por un mayor grado de irresponsabilidad. En consecuencia, es posible inferir que los barrios en los que se muestran los peores accidentes tengan la mayor cantidad de de estos en general (aplica para el caso contrario). De manera anólaga, en el caso de que lo anterior no se cumpla, con este dato por lo menos se podrá identificar los barrios con los accidentes más graves que, del mismo modo, servirá para prevenir a la ciudadanía y tratar de identificar de manera profunda lo que está pasando en el lugar. Al fin y al cabo los daños materiales importantes, pero reducir la cantidad de muertos en Medellín debe ser la prioridad.
...
df <- read.csv(file = "Analisis1.csv", header = T, nrows = 50,
               stringsAsFactors = T, fileEncoding="utf-8")
df; remove(df)
```
...
```{r, echo=FALSE, rows.print = 5}
df <- read.csv(file = "Analisis3.csv", header = T,
               stringsAsFactors = T, fileEncoding="utf-8")
df; remove(df)
```
...
Al realizar la respectiva clasificación se arrojó la siguiente gráfica, donde se puede observar la distribución de los barrios y se acepta la siguiente premisa: los 3 tipos de gravedad se relacionan entre sí, evidenciandose que a mayor numero de heridos, también hay mayor número de solo daños y muertos. En ese orden de ideas, se muestra otra información importante: Unos cúantos barrios son los que presentan la mayor concentración de accidentes, con una frecuencia mayor con respecto a los más seguros.

![](3tablas.jpeg){#id .class width=60% height=60%}

Teniendo en cuenta lo anterior, se analizó la media de accidentes por cluster, lo que arrojó una diferencia significativa entre ellos a la hora de analizar los heridos, muertos y daños ocasionados. El heatmap que se encuentra a continuación describe lo visto en las gráficas anteriores de una manera mas precisa. Por lo tanto, los números muestran una diferencia mucho mayor cuando se comparan los 2 grupos más sguros con los 2 peligrosos, donde visualmente se puede notar al igual que en los gráficos anteriores. Igualmente es importnte resaltar que en las 3 componentes, el promedio del cluster con más accidentes es mayor a la suma de los otros 3

Estos datos se muestran en la aplicación de shiny, en la parte de abajo de la sección de agrupamiento.

![](Heatmap.png){#id .class width=50% height=50%}
...
```{r, echo=FALSE, rows.print = 5}
df <- read.csv(file = "frecuencia.csv", header = T, nrows = 50,
               stringsAsFactors = T, fileEncoding="utf-8")
df; remove(df)
```
...
```{r, echo=FALSE, rows.print = 5}
df <- read.csv(file = "9barrios.csv", header = T, nrows = 50,
               stringsAsFactors = T, fileEncoding="utf-8")
df; remove(df)
```
...
![](mapa.png){#id .class width=50% height=50%}
...
# Referencias
  ...
  Andrew Gelman, Jennifer Hill (2007). Data Analysis Using Regression and
