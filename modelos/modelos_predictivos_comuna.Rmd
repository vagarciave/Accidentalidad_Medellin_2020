...
```{r}
y_est1_train_dia <- round(predict(mod1_dia, newdata = train_dia, type = "response"),0)
y_est1_test_dia <- round(predict(mod1_dia, newdata = test_dia, type = "response"),0)
MSE(train_dia$ACCIDENTALIDAD, y_est1_train_dia)
MSE(test_dia$ACCIDENTALIDAD, y_est1_test_dia)
```
```{r}
summary(train_dia$ACCIDENTALIDAD)
summary(y_est1_train_dia)
```
...
```{r}
base_semana <- base_dia %>% group_by(COMUNA, CLASE, PERIODO, SEMANA, TIEMPO_SEMANA) %>%
...
```{r}
base_mes <- base_dia %>% group_by(COMUNA, CLASE, PERIODO, MES, TIEMPO_MES) %>%
  summarise(DIA_FESTIVO = sum(DIA_FESTIVO), ACCIDENTALIDAD = sum(ACCIDENTALIDAD))
```
