...
```{r, message=FALSE, warning=FALSE}
mod_semana_comuna <- glmer(ACCIDENTALIDAD ~  DIA_FESTIVO + (1 | COMUNA/CLASE),
               data = train_semana_comuna, family= poisson())
save(mod_semana_comuna, file = "mod_semana_comuna.RData")
load(file = "mod_semana_comuna.RData")
```

```{r, message=FALSE, warning=FALSE}
summary(mod_semana_comuna)
```

```{r}
y_est_train_semana_comuna <- round(predict(mod_semana_comuna, newdata = train_semana_comuna, type = "response"),0)
y_est_test_semana_comuna <- round(predict(mod_semana_comuna, newdata = test_semana_comuna, type = "response"),0)
...
```{r, message=FALSE, warning=FALSE}
mod_mes_comuna <- glmer(ACCIDENTALIDAD ~  DIA_FESTIVO + (1 | COMUNA/CLASE),
               data = train_mes_comuna, family= poisson())
save(mod_mes_comuna, file = "mod_mes_comuna.RData")
load(file = "mod_mes_comuna.RData")
```

```{r, message=FALSE, warning=FALSE}
summary(mod_mes_comuna)
```

```{r}
y_est_train_mes_comuna <- round(predict(mod_mes_comuna, newdata = train_mes_comuna, type = "response"),0)
y_est_test_mes_comuna <- round(predict(mod_mes_comuna, newdata = test_mes_comuna, type = "response"),0)
...
base <- expand.grid(COMUNA = levels(accidentes_dia_comuna$COMUNA), CLASE = levels(accidentes_dia_comuna$CLASE),
                             FECHA = fecha_vector)
base <- base %>% mutate(TIEMPO_DIA = as.numeric(FECHA) -
                                            as.numeric(as.Date("2014-01-01")) + 1)
...
```{r}
base <- left_join(base, accidentes_dia_comuna, by = c("COMUNA", "CLASE", "FECHA", "TIEMPO_DIA",
                                       "PERIODO", "SEMANA", "TIEMPO_SEMANA", "MES",
                                       "TIEMPO_MES", "DIA_FESTIVO"))
base[is.na(base)] <- 0
```
